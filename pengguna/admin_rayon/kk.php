...
<body class="g-sidenav-show bg-gray-100">
    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="container-fluid py-4" id>
            ...
            // Ambil id_rayon berdasarkan id_admin_rayon
            ...
            $id_rayon = $row_rayon['id_rayon']; // Simpan id_rayon

            // Cek apakah ada pencarian dan pagination
            $search = isset($_GET['search']) ? $_GET['search'] : '';
            $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
            $limit = 10;
            $offset = ($page - 1) * $limit;

            // Query untuk mendapatkan data KK yang sesuai dengan id_rayon admin, dengan pencarian dan pagination
          ...
          WHERE kk.id_rayon = ? AND (kk.no_kk LIKE ? OR kk.nama_kepala_keluarga LIKE ?)
          LIMIT ?, ?";
            $stmt = $koneksi->prepare($query);
            $search_param = '%' . $search . '%';
            $stmt->bind_param("issii", $id_rayon, $search_param, $search_param, $offset, $limit);
            $stmt->execute();
            ...
            $total_query = "SELECT COUNT(*) as total
                FROM kk
                WHERE id_rayon = ? AND (no_kk LIKE ? OR nama_kepala_keluarga LIKE ?)";
            $stmt_total = $koneksi->prepare($total_query);
            $stmt_total->bind_param("iss", $id_rayon, $search_param, $search_param);
            $stmt_total->execute();
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="tambahForm" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="id_rayon" id="id_rayon" value="<?php echo $id_rayon; ?>">
