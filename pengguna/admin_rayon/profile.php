    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="container-fluid">
          ...
          ">
            ...
            // Periksa apakah session id_admin_rayon telah diset
            if (isset($_SESSION['id_admin_rayon'])) {
                $id_admin_rayon = $_SESSION['id_admin_rayon'];

                // Query SQL untuk mengambil data admin_rayon berdasarkan id_admin_rayon dari session
                $query = "SELECT * FROM admin_rayon WHERE id_admin_rayon = '$id_admin_rayon'";
                $result = mysqli_query($koneksi, $query);

                // Periksa apakah query berhasil dieksekusi
                if ($result) {
                    // Periksa apakah terdapat data admin_rayon
                    if (mysqli_num_rows($result) > 0) {
                        // Ambil data admin_rayon sebagai array asosiatif
                        $admin_rayon = mysqli_fetch_assoc($result);
            ?>
                        <div class="card card-body blur shadow-blur mx-4 mt-n6 overflow-hidden">
                            <div class="row gx-4">
                                <div class="col-auto">
                                    <div class="avatar avatar-xl position-relative">
                                        <a href="javascript:void(0)" onclick="document.getElementById('editFotoProfile').click()">
                                            <?php if (!empty($admin_rayon['fp'])): ?>
                                                <img src="../../assets/img/fp_pengguna/admin_rayon/<?php echo $admin_rayon['fp']; ?>"
                                ...
                                <div class="col-auto my-auto">
                                    <div class="h-100">
                                        <h5 class="mb-1"><?php echo $admin_rayon['nama']; ?></h5>
                                        <p class="mb-0 font-weight-bold text-sm"><?php echo $admin_rayon['nama']; ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body p-3">
                                <ul class="list-group">
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        ...
                                        <?php echo $admin_rayon['nama']; ?>
                                    </li>
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        <strong class="text-dark">Username:</strong> &nbsp;
                                        <?php echo $admin_rayon['username']; ?>
                                    </li>
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        <strong class="text-dark">Password:</strong> &nbsp;
                                        <?php echo $admin_rayon['password']; ?>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="editForm" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="id_admin_rayon"
                                value="<?php echo $admin_rayon['id_admin_rayon']; ?>" id="">
                            <div class="mb-3">
                                <label for="nama" class="form-label">Nama </label>
                                <input type="text" id="nama" name="nama" value="<?php echo $admin_rayon['nama']; ?>"
                                    class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label for="username" class="form-label">Username</label>
                                <input type="text" id="username" name="username"
                                    value="<?php echo $admin_rayon['username']; ?>" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label for="password" class="form-label">Password</label>
                                <input type="text" id="password" name="password"
                                    value="<?php echo $admin_rayon['password']; ?>" class="form-control" required>
                            </div>
...
<?php
                        ...
                        // Jika tidak ada data admin_rayon
                        echo "Tidak ada data admin_rayon.";
                    }
                } else {
                    // Jika query tidak berhasil dieksekusi
                    echo "Gagal mengambil data admin_rayon: " . mysqli_error($koneksi);
                }
            } else {
                // Jika session id_admin_rayon tidak diset
                echo "Session id_admin_rayon tidak tersedia.";
            }
