    ...
    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
        ...
        <div class="container-fluid">
          ...
          ">
            ...
            // Periksa apakah session id_ketua_mjh telah diset
            if (isset($_SESSION['id_ketua_mjh'])) {
                $id_ketua_mjh = $_SESSION['id_ketua_mjh'];

                // Query SQL untuk mengambil data ketua_mjh berdasarkan id_ketua_mjh dari session
                $query = "SELECT * FROM ketua_mjh WHERE id_ketua_mjh = '$id_ketua_mjh'";
                $result = mysqli_query($koneksi, $query);

                // Periksa apakah query berhasil dieksekusi
                if ($result) {
                    // Periksa apakah terdapat data ketua_mjh
                    if (mysqli_num_rows($result) > 0) {
                        // Ambil data ketua_mjh sebagai array asosiatif
                        $ketua_mjh = mysqli_fetch_assoc($result);
            ?>
                        <div class="card card-body blur shadow-blur mx-4 mt-n6 overflow-hidden">
                            <div class="row gx-4">
                                <div class="col-auto">
                                    <div class="avatar avatar-xl position-relative">
                                        <a href="javascript:void(0)" onclick="document.getElementById('editFotoProfile').click()">
                                            <?php if (!empty($ketua_mjh['fp'])): ?>
                                                <img src="../../assets/img/fp_pengguna/ketua_mjh/<?php echo $ketua_mjh['fp']; ?>"
                                ...
                                <div class="col-auto my-auto">
                                    <div class="h-100">
                                        <h5 class="mb-1"><?php echo $ketua_mjh['nama']; ?></h5>
                                        <p class="mb-0 font-weight-bold text-sm"><?php echo $ketua_mjh['nama']; ?></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body p-3">
                                <ul class="list-group">
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        ...
                                        <?php echo $ketua_mjh['nama']; ?>
                                    </li>
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        <strong class="text-dark">Username:</strong> &nbsp;
                                        <?php echo $ketua_mjh['username']; ?>
                                    </li>
                                    <li class="list-group-item border-0 ps-0 text-sm">
                                        <strong class="text-dark">Password:</strong> &nbsp;
                                        <?php echo $ketua_mjh['password']; ?>
                                    </li>
                                </ul>
                            </div>
                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="editForm" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="id_ketua_mjh" value="<?php echo $ketua_mjh['id_ketua_mjh']; ?>"
                                id="">
                            <div class="mb-3">
                                <label for="nama" class="form-label">Nama </label>
                                <input type="text" id="nama" name="nama" value="<?php echo $ketua_mjh['nama']; ?>"
                                    class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label for="username" class="form-label">Username</label>
                                <input type="text" id="username" name="username"
                                    value="<?php echo $ketua_mjh['username']; ?>" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label for="password" class="form-label">Password</label>
                                <input type="text" id="password" name="password"
                                    value="<?php echo $ketua_mjh['password']; ?>" class="form-control" required>
                            </div>
...
<?php
                        ...
                        // Jika tidak ada data ketua_mjh
                        echo "Tidak ada data ketua_mjh.";
                    }
                } else {
                    // Jika query tidak berhasil dieksekusi
                    echo "Gagal mengambil data ketua_mjh: " . mysqli_error($koneksi);
                }
            } else {
                // Jika session id_ketua_mjh tidak diset
                echo "Session id_ketua_mjh tidak tersedia.";
            }
